...
if(T){
  # select data
  data.sub = meta %>%
    filter(major_cluster %in% c("Epithelial")) %>%
    filter(stage == "adult") %>% 
  ...
  # order disease
  N.o = N.o[,c("Healthy","UC_inflamed","UC_non_inflamed")]
  ...
  R.oe.v2 = R.oe
  R.oe.v2[R.oe.v2 > 3] = 3
  pdf("scIBD.cell_type_compare.between_disease.epi.pdf", width = 3, height = 4)
  Heatmap(R.oe.v2,
          column_title = "",
...
if(T){
  tmp = meta %>% filter(stage == "adult") %>% 
    filter(major_cluster == "Epithelial") %>%
    filter(disease %in% c("Healthy","UC_inflamed")) %>%
  ...
  ggplot(tmp.filter.sub, aes(x=group, y = pct, fill = group, color = group)) + 
    ...
    scale_fill_manual(values = alpha(paletteer::paletteer_d("ggsci::category10_d3")[c(1,2)], 0.6) ) + 
    scale_color_manual(values = paletteer::paletteer_d("ggsci::category10_d3")[c(1,2)] ) + 
    scale_x_discrete(labels = c("HC","UC")) + 
    theme_classic2() + 
    theme(
  ...
  ggsave("DUOX2_epi.pct.boxplot.pdf", width = 1.8, height = 3)
  
  pval = t.test(tmp.filter.sub[tmp.filter.sub$group == "Healthy",]$pct, 
         tmp.filter.sub[tmp.filter.sub$group == "UC_inflamed",]$pct)
  pval = pval$p.value/2
  pval
}
  ...
  ggplot(tmp.filter.sub, aes(x=group, y = pct, fill = group, color = group)) + 
    ...
    theme(
      ...
      plot.title = element_text(family = "ArialMT",size = 18, color ='black', hjust = 0.5)
                           ...
                           method = "t.test", label = "p.signif", 
                           method.args = list(alternative = "greater")) 
  #ggsave("M_cell.pct.boxplot.pdf", width = 1.8, height = 3)
