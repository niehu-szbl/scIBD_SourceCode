...
if(T){
  # select data
  data.sub = meta %>%
    filter(major_cluster %in% c("Endothelial")) %>%
    filter(stage == "adult") %>% 
...
# Adult venous EC (SELE+)
if(T){
  tmp = meta %>% filter(stage == "adult") %>% 
    filter(major_cluster == "Endothelial") %>%
  ...
  tmp.filter %>% group_by(disease) %>% count() # CD_inflamed 2, Healthy 16, UC_inflamed 17
  ...
  # ggsave("Adult_venous_EC_SELE.pct.boxplot.pdf", width = 2, height = 3)
  ...
  ggplot(tmp.filter.sub, aes(x=group, y = pct, fill = group, color = group)) + 
    ...
    theme(
      ...
      plot.title = element_text(family = "ArialMT",size = 18, color ='black', hjust = 0.5)
                           ...
                           method = "t.test", label = "p.signif", 
                           method.args = list(alternative = "less")) 
  # ggsave("Cycling_EC.pct.boxplot.pdf", width = 2, height = 3)
