...
## read data
obs = pd.read_csv("../ibd_20220111.bbknn_umap_leiden_marker.adata.obs.csv")
cells = obs.iloc[:,0][ obs.label.isin(["B cells", "Plasma cells"]) ].to_list()

## subset cells
adata = sc.read_h5ad("../ibd_20220111.raw.h5ad")
adata = adata[cells]

## special gene clusters
IG = pd.read_table("/home/niehu/niehu/database/00.genes/excluded/ig2.txt", header = None)
...
exclude = IG + TCR + mt + rp ## keep IGH (not IGHV)
...
## find hvg
select = adata.obs.sampleName.value_counts() > 100
select = select.index[select].to_list()
...
adata.write_h5ad("B_plasma.bySample.n10.h5ad")
